...
//     WEST_LON,EAST_LON,TOTAL_SIZE,BASE_URL
    ...
    .flatMap(line => {
      ...
      (tokens(0), tokens(1), tokens(2), tokens(3), tokens(4), tokens(5), tokens(6), tokens(7), tokens(8), tokens(9), tokens(10), tokens(11), tokens(12), tokens(13), tokens(14), tokens(15), tokens(16),tokens(17))
    })
    ...
    .map(line => {
      ...
      var no = 0
      var na = 0
      if(line._17 == "Yes") yes = 1
      else if(line._17 == "No") no = 1
      else na = 1
      (line._9, (yes, no, na))
    })
    .reduceByKey((v1,v2) => {
